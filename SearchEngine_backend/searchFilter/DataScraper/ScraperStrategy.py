...
class GoogleSearchStrategy(ScraperStrategy):
    ...
    def parse_results(self, html_content: str) -> list:
        ...
        print(f"[Google] HTML length: {len(html_content)}")

        # Save full HTML dump for debugging
        try:
            with open("google_results.html", "w", encoding="utf-8") as f:
                f.write(html_content)
            print("[Google] Wrote full HTML to google_results.html")
        except Exception as e:
            print(f"[Google] Failed to write HTML: {e}")
        ...
        def _clean_href(href: Optional[str]) -> Optional[str]:
            ...
            if href.startswith("/aclk"):
                return None  # ad click tracker
            if href.startswith("http") and "google.com" not in href:
                return href
            if href.startswith("/"):
                return None  # internal Google path
            return None

        def _is_ad(element) -> bool:
            html_str = str(element)
            signals = ["sponsored", "/aclk", "data-text-ad", "Sponsored"]
            return any(s in html_str for s in signals)

        def _add(title: str, link: str, desc: str, is_ad: bool):
            if not title or not link:
                return
            if link in seen:
                return
            seen.add(link)
            tag = "AD" if is_ad else "ORG"
            print(f"[Google][{tag}] {title} -- {link}")
            results.append({
                "searchEngine": "Google",
                "baseUrl": BASE,
                "title": title,
        ...
        # ── Strategy 1: classic div.g ──────────────────────────────────────
        cards = soup.select("div.g")
        print(f"[Google] Strategy 1 (div.g): {len(cards)} cards")
        for card in cards:
            a = card.select_one("a[href]")
            h3 = card.select_one("h3")
            desc_tag = card.select_one("div.VwiC3b, div[style*='-webkit-line-clamp'], span.st")
            if not (a and h3):
                continue
            link = _clean_href(a.get("href", ""))
            if not link:
                continue
            _add(h3.get_text(strip=True), link,
                 desc_tag.get_text(strip=True) if desc_tag else "", _is_ad(card))

        # ── Strategy 2: #rso container ─────────────────────────────────────
        if not results:
            rso_cards = soup.select("#rso > div, #rso .hlcw0c, #rso .MjjYud > div")
            print(f"[Google] Strategy 2 (#rso children): {len(rso_cards)} cards")
            for card in rso_cards:
                a = card.select_one("a[href]")
                h3 = card.select_one("h3")
                desc_tag = card.select_one("div.VwiC3b, span.st")
                if not (a and h3):
                    continue
                link = _clean_href(a.get("href", ""))
                if not link:
        ...
        # ── Strategy 3: any <a> wrapping an <h3> ──────────────────────────
        if not results:
            print("[Google] Strategy 3: scanning all <a><h3> pairs")
            for a in soup.find_all("a", href=True):
                ...
                desc_tag = parent.find("span") if parent else None
                desc = desc_tag.get_text(strip=True) if desc_tag else ""
                _add(h3.get_text(strip=True), link, desc, _is_ad(a))

        # ── Strategy 4: data-hveid attribute (Google result containers) ────
        if not results:
            print("[Google] Strategy 4: data-hveid divs")
            for card in soup.find_all("div", attrs={"data-hveid": True}):
                ...
                if not link:
                    continue
                _add(h3.get_text(strip=True), link, "", _is_ad(card))

        print(f"[Google] Total results parsed: {len(results)}")
        return results
